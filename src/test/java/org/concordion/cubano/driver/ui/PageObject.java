 ...
 * 
 * @param <T>
 *            The class of the PageObject that is extending this class.
 * 
 * @author sumnera
 */
public abstract class PageObject<T extends PageObject<T>> extends BasePageObject<T> {
	public PageObject(BrowserBasedTest test) {
		this(test, WebDriverConfig.getInstance().getPropertyAsInteger("webdriver.timeouts.implicitlywait", "5"));
	}
	...
	/**
	 * Returns a condition that checks whether the page is loaded. This is called
	 * when constructing a new PageObject to check that it is in sync with the
	 * browser.
	 * <p/>
	 * Since WebDriver can return prior to a page being fully loaded, this condition
	 * must be unique to the page being loaded and must not be met by the previous
	 * page that we are transferring from.
	 * 
	 * @param params
	 *            List of parameters that where passed in to the constructor
	...
	protected void waitUntilPageIsLoaded(int timeoutWaitInSeconds, Object... params) {
		...
		ActionTimer timer = ActionTimer.start(getLogger(), "Checking {} loads within {} seconds", getSimpleName(),
				timeoutWaitInSeconds);
	...
	/**
	 * As MyMSD is a single page application WebDriver does not wait for a "new
	 * page" to load when click on button/link, instead the Ajax request is fired
	 * off and WebDriver passes control back to the test. The will ensure that any
	 * Ajax requests have completed before moving on.
	 * 
